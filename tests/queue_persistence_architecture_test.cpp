...
int main() {
    ...
    if (archive_h.empty() || archive_cpp.empty() || live_h.empty() || live_export.empty() ||
        window_h.empty() || window_cpp.empty() || persistence.empty() ||
        project.empty() || cmake.empty()) {
        return fail(1, "required production source missing");
    }

    if (!contains(archive_h, "current_plan") ||
        !contains(archive_h, "current_append_jobs") ||
        !contains(archive_h, "queued_jobs")) {
        return fail(2, "archive must preserve current session and future sessions separately");
    }

    if (!contains(archive_cpp, "VCQUEUE1") ||
        !contains(archive_cpp, "kFormatVersion = 1") ||
        !contains(archive_cpp, "kMaxEntries") ||
        !contains(archive_cpp, "TempFileGuard") ||
    ...
    if (!contains(live_h, "export_remaining_plan() const") ||
        ...
        return fail(4, "restartable plan must export active files before pending files");
    }

    if (!contains(window_cpp, "ConfigureQueuePersistenceMenu()") ||
        !contains(persistence, "FileSavePicker") ||
        !contains(persistence, "FileOpenPicker") ||
        !contains(persistence, "resume_background()") ||
        !contains(persistence, "ActionQueueOptions") ||
        !contains(persistence, "AutomationProperties::SetName")) {
        return fail(5, "WinUI persistence commands must be compact, native and accessible");
    }
    ...
    if (!contains(persistence, "revalidate_plan_sources") ||
        ...
        !contains(persistence, "merged.append(planner.build(job), true)")) {
        return fail(6, "load must revalidate sources and restore same-session appends");
    }

    if (!contains(persistence, "StartCopyPlan") ||
        !contains(persistence, "RunLivePlanSession") ||
        contains(persistence, "executor_.execute(")) {
        return fail(7, "loaded plans must reuse the one production execution loop");
    }

    if (!contains(persistence, "!execution_control_ && !live_plan_") ||
        !contains(persistence, "!stopped_session_") ||
        !contains(persistence, "!conflict_session_") ||
        !contains(persistence, "queued_sessions_.empty()")) {
        return fail(8, "loading must remain disabled while another session exists");
    }
    ...
    if (!contains(project, "MainWindow.QueuePersistence.cpp") ||
        ...
        return fail(9, "persistence production units/tests must be compiled");
    }

    if (contains(window_h, "queue_archive_store_")) {
        return fail(10, "unused persistence state must not remain in MainWindow");
    }
